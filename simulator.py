...
maxIttr = 300  # how many time steps we want to simulate
...
"""
epsilon = 0.2 # Noise of 0.2 good
samplingN = 150 # Only for VO
...
def readScenario(fileName, scalex=1., scaley=1.):
    ...
    for line in lines: # create an agent and add it to the list
        if method=="SFM":
            agents.append(SFMAgent(line.split(','), 1, 10, 0.5, 5000, 0.08, 1.2e5, 2.4e5, 80, 500)) 
        elif method=="TTC":
            agents.append(TTCAgent(line.split(','), 1, 10, 0.5, 5, epsilon, 10, 1)) 
        elif method=="VO":
            agents.append(VOAgent(line.split(','), 1, 5, epsilon, samplingN, 1, 1, 5)) 
    ...
    x_min =	min(np.amin(np.array(positions)[:,0]), np.amin(np.array(goals)[:,0]))*scalex - 3.
    y_min =	min(np.amin(np.array(positions)[:,1]), np.amin(np.array(goals)[:,1]))*scaley - 3.
    x_max =	max(np.amax(np.array(positions)[:,0]), np.amax(np.array(goals)[:,0]))*scalex + 3.
    y_max =	max(np.amax(np.array(positions)[:,1]), np.amax(np.array(goals)[:,1]))*scaley + 3.
...
if doExport:
    ...
    exportFile = scenarioFile.split('.csv')[0] + "_" + method + "_epsilon_" + str(epsilon) + "_sim.csv"
